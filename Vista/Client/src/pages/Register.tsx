...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nombreCompleto = formData.name.trim().split(" ");
      const nombre = nombreCompleto[0];
      const apellido = nombreCompleto.slice(1).join(" ") || nombre;

      // URL de tu API - AJUSTA ESTA RUTA
      const API_URL = "http://localhost/agencia-project/controlador/usuarios.php"; // ⚠️ CAMBIA ESTO

      console.log("Intentando registrar usuario...");
      console.log("Datos:", { nombre, apellido, email: formData.email, empresa: formData.empresa });

      // Paso 1: Crear usuario directamente (sin crear cliente primero)
      const usuarioResponse = await fetch(`${API_URL}/usuarios.php?insertar`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          usu_cli_id: 1, // ID de cliente por defecto, o null si lo permites
          usu_nombre: nombre,
          usu_apellido: apellido,
          usu_email: formData.email,
          usu_password: formData.password,
      ...
      console.log("Response status:", usuarioResponse.status);

      // Verificar si la respuesta es JSON válido
      const contentType = usuarioResponse.headers.get("content-type");
      if (!contentType || !contentType.includes("application/json")) {
        const textResponse = await usuarioResponse.text();
        console.error("Respuesta no es JSON:", textResponse);
        setError("Error en el servidor. Verifica la configuración de la API.");
        setIsLoading(false);
        return;
      }

      const usuarioData = await usuarioResponse.json();
      console.log("Respuesta del servidor:", usuarioData);

      if (usuarioData.success === 1) {
        console.log("Usuario creado exitosamente, intentando login...");
        
        // Login automático
        const loginResponse = await fetch(`${API_URL}/usuarios.php?login`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (loginData.success === 1) {
          ...
          console.log("Login falló, redirigiendo a /login");
          // Si falla el login automático, redirigir a login
          navigate("/login");
        }
      } else {
        console.error("Error al crear usuario:", usuarioData);
        setError(usuarioData.message || "Error al crear usuario. Intenta nuevamente.");
      }
    } catch (error) {
      ...
      if (error instanceof TypeError && error.message.includes("Failed to fetch")) {
        setError("No se puede conectar con el servidor. Verifica que el backend esté funcionando y la URL sea correcta.");
      } else {
