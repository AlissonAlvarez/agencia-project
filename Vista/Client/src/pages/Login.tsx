...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ⚠️ NOTA: Agregado el parámetro ?login a la URL
      const response = await fetch("http://localhost/agencia-project/controlador/usuarios.php?login", {
        method: "POST",
        headers: {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        setError("Error en el servidor. Respuesta inválida.");
        setIsLoading(false);
        return;
      }
      ...
      if (data.success === 1) {
        ...
        // Guardar datos del usuario en el contexto/localStorage
        login(data.usuario);
      ...
      if (error instanceof TypeError && error.message.includes("Failed to fetch")) {
        setError("No se puede conectar con el servidor. Verifica que Apache esté funcionando.");
      } else {
